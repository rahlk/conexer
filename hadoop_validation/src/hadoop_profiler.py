...
class Profiler:
    def __init__(self):
        ...
        self.hadoop_conf_home = os.sep.join(
            [str(cfg.hadoop_home), 'etc', 'hadoop'])
        self.avg_run_time = 500000
        self.hibench_output = ''
    ...
    def parse_orig_confs(self):
        ...
        files = ['mapred-site.xml', 'core-site.xml',
    ...
    def backup_original_confs(self):
        ...
        '''
        # confs = []
        conf_files = ['mapred-site.xml', 'core-site.xml',
                      'yarn-site.xml', 'hdfs-site.xml']
        for conf_file in conf_files:
            if cfg.platform == 'aws':
                cmd = ' '.join(['sudo cp', self.hadoop_conf_home +
                                os.sep + conf_file, self.backup_folder])
            else:
                cmd = ' '.join(['cp', self.hadoop_conf_home +
                                os.sep + conf_file, self.backup_folder])
            self.run_cmd(cmd)
    ...
    def profile(self, itertime, in_conf):
        ...
        self.curr_genconf_folder = cfg.gen_confs + \
            os.sep + 'conf' + str(self.itertime)
    ...
    def wait_timeout(self, proc, seconds):
        ...
        while True:
            ...
            if time.time() > end:
                ...
                print 'wait timeout:', seconds, 'kill process...'
    ...
    def kill_useless_process(self):
        ...
        slave_nodes = ['hadoop-worker1', 'hadoop-worker2',
                       'hadoop-worker3', 'hadoop-worker4']
        for s in slave_nodes:
            kill_python = 'ssh ' + s + \
                ' \"ps -ef|grep \'socket.SOCK\'|awk \'{ print \$2 }\' |xargs kill\"'
    ...
    def restart_hadoop_with_new_conf(self, confs):
        ...
        if cfg.platform == 'docker':
            ...
            start_all = [os.sep.join(
                [cfg.hadoop_home, 'sbin', 'start-all.sh'])]
    ...
    def restore_hadoop_confs(self):
        files_to_copy = [self.backup_folder + os.sep +
                         f for f in os.listdir(self.backup_folder)]
        if cfg.platform == 'aws':
            cmd = ' '.join(['sudo cp', ' '.join(
                files_to_copy), self.hadoop_conf_home])
        else:
            cmd = ' '.join(
                ['cp', ' '.join(files_to_copy), self.hadoop_conf_home])
        self.run_cmd(cmd)
    ...
    def copy_new_conf(self, confs):
        ...
        if cfg.platform == 'aws':
            copy_cmd = ' '.join(
                ['sudo cp', files_to_copy, master_target_folder])
        else:
